...
# โหลดเฉพาะ pair_13 จาก gold dataset
...
resume_data = extract_resume(item["resume_text"], model=MODEL_NAME)
print(resume_data.model_dump_json(indent=2, ensure_ascii=False))

print("\n=== STEP 2: JD Extractor ===")
jd_data = extract_jd(item["jd_text"], model=MODEL_NAME)
print(jd_data.model_dump_json(indent=2, ensure_ascii=False))
print(f"\nจำนวน JD Requirements สกัดได้: {len(jd_data.requirements)} รายการ")
...
fit_result = analyze_fit(resume_data, jd_data, model=MODEL_NAME)
...
verified = judge_matches(fit_result.matches, item["resume_text"], model=MODEL_NAME)
for m in verified:
